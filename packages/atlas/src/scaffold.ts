...
function mergePackageJson(raw: string | null, atlasDependency: string): string {
  ...
  const existingAtlas = typeof dependencies['@mirai/atlas'] === 'string'
    ? dependencies['@mirai/atlas']
    : typeof devDependencies['@mirai/atlas'] === 'string'
      ? devDependencies['@mirai/atlas']
      : null;
  if (existingAtlas !== null && existingAtlas !== atlasDependency) {
    throw new AtlasCliError('CONFLICT', `Existing @mirai/atlas dependency ${existingAtlas} conflicts with required ${atlasDependency}`, {
      nextAction: 'Run atlas upgrade or align the existing @mirai/atlas dependency before adoption',
    });
  }
  if (dependencies['@mirai/atlas'] === undefined) devDependencies['@mirai/atlas'] = atlasDependency;
  const merged = {
    ...pkg,
