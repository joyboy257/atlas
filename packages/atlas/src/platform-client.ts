...
export class AtlasPlatformClient {
  ...
  async inspectMcp(): Promise<{ protocol: string; session: boolean; tools: Record<string, unknown>[]; resources: Record<string, unknown>[]; collisions: string[] }> {
    const initialized = await this.mcpRpc({ jsonrpc: '2.0', id: 'atlas-cli-initialize', method: 'initialize', params: { protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: '@atlas-runner/atlas', version: '0.1.0-alpha.0' } } });
